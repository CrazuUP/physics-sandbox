(() => {
    ...
    function render() {
        ...
        // "Спицы" блока (для визуализации вращения)
        ctx.save();
        ctx.translate(px, py);
        // ctx.rotate(state.pos / state.R_p); // Вращение в зависимости от позиции (не реализовано полностью)
        ctx.strokeStyle = '#555';
        ctx.lineWidth = 2;
        for (let i = 0; i < 6; i++) {
            ctx.rotate(PI / 3);
        ...
        if (state.countdown > 0) {
            ...
            ctx.fillText(Math.ceil(state.countdown).toString(), W/2, H/2);
            ctx.textAlign = 'left';
